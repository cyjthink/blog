    ...
    private void playSendGiftAnimator(SendGiftModel sendGiftModel) {
        ...
        if (null == samePlayingLayout) {
            ...
            if (null == freeLayout) {
                ...
                // 当giftNum > 1时，即是从队列中取出的，需要重复播放
                if (giftNum > 1) {
                    Log.e(TAG, "当前giftNum > 1, giftNum = " + giftNum);
                    animatorSet = freeLayout.playAnimatorSet(freeLayout.playEnterAnimator(),
                            freeLayout.playRepeatScaleTextAnimator(giftNum));
                } else {
                    animatorSet = freeLayout.playAnimatorSet(freeLayout.playEnterAnimator(),
                            freeLayout.playScaleTextAnimator(giftNum));
                }
                animatorSet.start();
                freeLayout.postGiftMessageDelayed(1000 + 500 * giftNum);
            }
